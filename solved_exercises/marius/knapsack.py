...
def knapsack(capacity, items):
    items_by_value = sorted(items, key=lambda x: x["value"], reverse=True)
    ...
    while index < len(items_by_value):
        if remaining_capacity - items_by_value[index]["weight"] >= 0:
            knapsack_items.append(items_by_value[index])
            total_value += items_by_value[index]["value"]
            total_weight += items_by_value[index]["weight"]
            remaining_capacity -= items_by_value[index]["weight"]
        index += 1
    return {"capacity": capacity, "items": knapsack_items, "weight": total_weight, "value": total_value}

print(knapsack(5,items))
print(knapsack(0,items))
